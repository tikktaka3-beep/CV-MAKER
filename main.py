...
Oqim:
1. Foydalanuvchi matn/mavzu yuboradi
2. Bot tilni so'raydi (uz/ru/en)
3. Bot slayd sonini so'raydi (5-8)
4. Bot uslubni so'raydi (rasmiy/quvnoq/simple)
5. Bot qo'shimcha izoh bor-yo'qligini so'raydi
6. Kunlik limit tekshiriladi -> AI orqali tarkib yaratiladi -> .pptx tayyorlanadi -> yuboriladi
"""
...
def kb_style() -> InlineKeyboardMarkup:
    return InlineKeyboardMarkup(inline_keyboard=[
        ...
        [InlineKeyboardButton(text="😄 Quvnoq", callback_data="style_quvnoq")],
        [InlineKeyboardButton(text="✏️ Simple", callback_data="style_simple")],
    ])
...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.answer(
        ...
        "Menga mavzu yoki matn yuboring — men 5-8 slayddan iborat chiroyli "
...
async def cmd_help(message: Message):
    await message.answer(
        ...
        "4. Uslubni tanlang\n"
        "5. Qo'shimcha izoh kiritish yoki o'tkazib yuborish\n\n"
...
async def choose_count(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer("🎨 Taqdimot uslubini tanlang:", reply_markup=kb_style())
...
async def choose_style(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.answer(
        "Agar bo'lsa yozing (masalan: 'statistik raqamlar bo'lsin'), "
...
# ---------- Holatdan tashqari matn (har qanday vaqt yangi mavzu yuborish) ----------
@dp.message(F.text)
